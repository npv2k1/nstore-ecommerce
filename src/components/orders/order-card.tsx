...
const OrderCard: React.FC<OrderCardProps> = ({ onClick, order, isActive }) => {
  ...
  const { tracking_number, status, created_at, delivery_time } = order;
    ...
    >
      <div className="flex justify-between items-center border-b border-border-200 py-3 px-5 md:px-3 lg:px-5 ">
        <span className="flex font-bold text-sm lg:text-base text-heading me-4 flex-shrink-0">
          {t('text-order')}
          <span className="font-normal">#{"..."+tracking_number.slice(tracking_number.length-5)}</span>
        </span>
        <span
          className={`text-sm text-white bg-blue-100 px-3 py-2 rounded whitespace-nowrap truncate max-w-full`}
          style={{
